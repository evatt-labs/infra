...
resource "namecheap_domain_records" "evattlabs_com" {
  ...
  mode        = "OVERWRITE"
  nameservers = data.terraform_remote_state.evattlabs_dns.outputs.evattlabs_com_name_servers
}
...
resource "namecheap_domain_records" "kraai_dev" {
  domain      = "kraai.dev"
  mode        = "OVERWRITE"
  nameservers = data.terraform_remote_state.kraai_dns.outputs.kraai_dev_name_servers
}
